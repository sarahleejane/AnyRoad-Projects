#***Analysing booking patterns for all tours that were not booked directly through AnyRoad
...
#**Clean data to get indirect AnyRoad bookings
data_excl_sg <- subset(bookings.users.tours.slots, bookings.users.tours.slots$guide_id != 732)
data_excl_sg <- 
  (subset(data_excl_sg, (data_excl_sg$referred_from!="" |
             data_excl_sg$referrer_token!="" |
               data_excl_sg$creative == "f") &
             !grepl("anyroad", data_excl_sg$referred_from)))
...
head(table_users2_slug,10)
#*** pause *** - am I taking goof number of tops?
table_users2_slug <- subset(table_users2_slug, table_users2_slug$Freq > 7)
