#***Analysing booking patterns for all St George tours
...
#*** pause ***
table_users2_slug <- subset(table_users2_slug, table_users2_slug$Freq > 1)
...
library(zoo)
per_month <- as.data.frame(data_excl_sg$date.formatted)
colnames(per_month) <- 'Var1'
per_month$Var1 <- as.yearmon(per_month$Var1, "%b %Y")
per_month$Var1 <- format(per_month$Var1, "%m-%Y")
table(per_month$Var1)
